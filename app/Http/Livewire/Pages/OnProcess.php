...
{use WithPagination;
    ...
    public function selectedTransaction($id, $action){
        ...
        if($action == 'delete'){
            ...
            $this->dispatchBrowserEvent('openProccedModal');
        }
    }
    ...
    public function procced(){
        Transaction::find($this->idtransaction)->update(['status' => 'Done']);
        $this->dispatchBrowserEvent('closeProccedModal');
        $this->dispatchBrowserEvent('notyf:success',['message' => 'Pesanan berhasil diselesaikan']);
    }
    public function proccedThenPrint(){
        Transaction::find($this->idtransaction)->update(['status' => 'Done']);
        $this->dispatchBrowserEvent('closeProccedModal');
        $this->dispatchBrowserEvent('notyf:success',['message' => 'Pesanan berhasil diselesaikan']);
        $this->print();
    ...
    {
        $datas = [
            'transactions' => Transaction::whereStatus('On Process')->latest()->paginate(5),
        ];
        return view('livewire.pages.on-process', $datas);
    }
}
